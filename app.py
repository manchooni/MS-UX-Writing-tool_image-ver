    ...
    with st.spinner("결과를 생성하는 중..."):
        ...
        if uploaded_image:
            ...
            try:
                ...
                **변경 전, 변경 후, 사유를 표(Markdown Table)로 보여줍니다. 표의 헤더는 '변경 전', '변경 후', '사유'로 합니다. 사항은 단위를 쪼개서 부분부분 설명합니다.**
                ...
                3. 규칙 위반 사항
                **위반된 규칙과 원칙을 리스트로 최대3개 보여줍니다. 위반된 규칙과 해당 텍스트 원문을 콜론(:)으로 구분하여 설명합니다.**
        ...
        # 2. 텍스트만 처리: 이미지 없이 텍스트 입력만 있을 경우 기존 로직 유지
            ...
            if mode == "일반 모드 (Gemini)":
                prompt_gemini = f"""
                당신은 항공사 UX 라이터입니다. 사용자가 보낸 문장을 명확하고 신뢰가 가는 긍정적인 화법으로 전환율을 높이는 문장으로 수정해 주세요.
                ...
                **변경 전, 변경 후, 사유를 표(Markdown Table)로 보여줍니다. 표의 헤더는 '변경 전', '변경 후', '사유'로 합니다. 사항은 단위를 쪼개서 부분부분 설명합니다.**
                ...
                3. 규칙 위반 사항
                **위반된 규칙과 원칙을 최대3개 리스트로 보여줍니다. 위반된 규칙과 해당 텍스트 원문을 콜론(:)으로 구분하여 설명합니다.**
