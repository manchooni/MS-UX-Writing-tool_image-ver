...
당신은 항공사 웹/앱 전문 UX 라이터입니다. 대한항공의 보이스톤을 기반으로 사용자 경험을 최적화하는 마이크로카피를 작성합니다.
    ...
    with st.spinner("결과를 생성하는 중..."):
        ...
        if uploaded_image:
            ...
            try:
                ...
                # --- [Gemini Prompt START] ---
                prompt_gemini = f"""
                당신은 UX 라이터입니다. 제공된 텍스트와 사용자 요청을 기반으로, 명확하고 신뢰가 가는 긍정적인 화법으로 전환율을 높이는 문장으로 수정해 주세요.
                ...
                **각 항목(검토요청문장, 검토 결과 제안1, 제안2)을 제목과 콜론(:)으로 명확히 구분하여 보여줍니다. 각 항목은 줄바꿈하여 나열합니다.**
                ...
                2. 변경 사항
                **변경 전, 변경 후, 사유를 표(Markdown Table)로 보여줍니다. 표의 헤더는 '변경 전', '변경 후', '사유'로 합니다.**
                
                3. 규칙 위반 사항
                **위반된 규칙과 원칙을 항목별로 보여줍니다. 위반된 규칙과 해당 텍스트 원문을 콜론(:)으로 구분하여 설명합니다.** (최대3개)
        ...
        # 2. 텍스트만 처리: 이미지 없이 텍스트 입력만 있을 경우 기존 로직 유지
            ...
            if mode == "일반 모드 (Gemini)":
                prompt_gemini = f"""
                당신은 UX 라이터입니다. 사용자가 보낸 문장을 명확하고 신뢰가 가는 긍정적인 화법으로 전환율을 높이는 문장으로 수정해 주세요.
                ...
                **변경 전, 변경 후, 사유를 표(Markdown Table)로 보여줍니다. 표의 헤더는 '변경 전', '변경 후', '사유'로 합니다.**
                ...
                3. 규칙 위반 사항
                **위반된 규칙과 원칙을 항목별로 번호를 매겨 보여줍니다. 위반된 규칙과 해당 텍스트 원문을 콜론(:)으로 구분하여 설명합니다.** (최대3개)
