...
요소별
- 대화형 버튼은 서로 호응하도록.
- 버튼은 명사형으로 종결하는 것을 원칙으로 합니다. 동사의 경우는 ‘-기’를 사용합니다.
    ...
    with st.spinner("결과를 생성하는 중..."):
        ...
        if uploaded_image:
            ...
            try:
                ...
                **각 항목(검토요청문장, 검토 결과 제안1, 제안2)을 제목과 콜론(:)으로 명확히 구분하여 보여줍니다. 각 항목은 한 줄씩 나열하며, 각 항목 뒤에는 반드시 빈 줄(줄바꿈 두 번)을 넣어 단락을 구분하세요.**
                ...
                **최종문장은 bold처리합니다. 부가설명 앞에는 반드시 빈 줄(줄바꿈 두 번)을 넣어 단락을 구분하세요.**
        ...
        # 2. 텍스트만 처리: 이미지 없이 텍스트 입력만 있을 경우 기존 로직 유지
            ...
            if mode == "일반 모드 (Gemini)":
                ...
                **최종문장은 bold처리합니다. 최종 추천문장 뒤, 부가설명 앞에는 반드시 빈 줄(줄바꿈 두 번)을 넣어 단락을 구분하세요.**
                규칙을 모두 준수하는 문장을 제시하세요. 
